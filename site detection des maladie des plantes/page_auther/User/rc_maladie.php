  ...
  echo $e->getMessage();
   ...
   foreach ($stmt as $row) {
    $nbr2++;
      ...
      $nbr=shell_exec("python C:\\wamp\\www\\a_stage_site\\page_auther\\User\\rc_maladie.py  ".$nbr1." ".$nbr2."  ");
     $req = "SELECT * FROM maladie";
     $sel=0;
   $stmt=$pdo->query($req);
   foreach ($stmt as $row) {
      if($sel==$nbr){
        $_SESSION['upmaladie']=$row['nom_maladie'];
        echo "<script>
        window.location='afficher_rc_maladie.php';
        </script>";
     ...
     <div class="row">
        ...
        <section class="page-section portfolio" id="biblio" style="margin-top: -80px;">
            <div class="container">
				...
				<hr>
                    <h4 class="page-section-heading text-secondary mb-0 d-inline-block"> Reconnaissance des maladies    </h4>
					<hr>
                </div>
